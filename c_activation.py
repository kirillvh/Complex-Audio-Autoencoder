...
class GenericComplexActivation(Module):
    def __init__(self, activation, use_phase: bool = False):
        '''
        activation can be either a function from nn.functional or an object of nn.Module if the activation has learnable parameters
...
class CReLU(Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        return crelu(input, inplace=self.inplace)

    def extra_repr(self) -> str:
        inplace_str = 'inplace=True' if self.inplace else ''
        return inplace_str
    
def cgelu(input: Tensor, inplace: bool = False) -> Tensor:
    '''
    
    '''
    if input.is_complex():
        return torch.view_as_complex(torch.stack((F.gelu(input.real), F.gelu(input.imag)),dim=-1))
    else:
        return F.gelu(input, inplace=inplace)
...
class CGeLU(Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        return cgelu(input, inplace=self.inplace)
